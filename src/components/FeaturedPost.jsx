...
export default function FeaturedPost({ post }) {
    ...
    return (
        <Link className="bg-[#EFCC5F] flex us:h-48 sm:h-64 md:h-96 overflow-hidden cursor-pointer mb-9" href={post.slug}>
            ...
            <div className="post_container text-[--nukasa_purple] mr-2 sm:ml-20 sm:mt-6 md:mt-12 us:ml-4">
                <h2 className="post_title us:text-2xl md:text-6xl lg:text-8xl lg:pl-12">{post.title}</h2>
                <div className="post_excerpt us:text-xs us:mt-8 sm:pl-8 sm:pr-4 sm:text-base sm:mb-12 sm:mt-12 md:text-xl lg:text-2xl lg:mt-12 md:mb-16 md:pl-24 md:mt-16">{post.excerpt}</div>
                
                <div className="uppercase font-[Oswald] us:text-xs mt-1 mb-1 us:mt-12 md:mt-12 md:text-base md:pl-12 "> by {post.author.name}</div>
            </div>
        </Link>
    )
}